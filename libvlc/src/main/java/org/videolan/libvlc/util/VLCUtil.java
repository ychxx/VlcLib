...
public class VLCUtil {
    ...
    public static boolean hasCompatibleCPU(Context context) {
        ...
        if (lib != null && (elf = readLib(lib)) != null) {
            ...
            VlcLog.i(TAG, "ELF ABI = " + (elfHasArm ? "arm" : elfHasX86 ? "x86" : "mips") + ", " +
                    (elfIs64bits ? "64bits" : "32bits"));
            VlcLog.i(TAG, "ELF arch = " + elf.att_arch);
            VlcLog.i(TAG, "ELF fpu = " + elf.att_fpu);
        } else {
            VlcLog.w(TAG, "WARNING: Unable to read libvlcjni.so; cannot check device ABI!");
        }
        ...
        if (elf != null) {
            ...
            if (elfHasX86 && !hasX86) {
                ...
                if (hasPlaceHolder && isIntel) {
                    VlcLog.d(TAG, "Emulated armv7 detected, trying to launch x86 libraries");
                } else {
        ...
        try {
            ...
            VlcLog.w(TAG, "Could not find maximum CPU frequency!");
        } catch (NumberFormatException e) {
            VlcLog.w(TAG, "Could not parse maximum CPU frequency!");
            VlcLog.w(TAG, "Failed to parse: " + line);
        } finally {
            if (br != null)
                try {
                    br.close();
        ...
        VlcLog.d(TAG, "machineSpecs: hasArmV6: " + hasArmV6 + ", hasArmV7: " + hasArmV7 +
    ...
    private static File searchLibrary(ApplicationInfo applicationInfo) {
        ...
        if (libraryPaths[0] == null) {
            VlcLog.w(TAG, "can't find library path");
            return null;
        }
        ...
        VlcLog.w(TAG, "WARNING: Can't find shared library");
        return null;
    }
    ...
    private static boolean readHeader(RandomAccessFile in, ElfData elf) throws IOException {
        ...
        if (bytes[0] != 127 ||
                ...
                bytes[3] != 'F' ||
                (bytes[4] != 1 && bytes[4] != 2)) {
            VlcLog.w(TAG, "ELF header invalid");
            return false;
        }
